...
/**
 * FXML Controller class for the CustSched FXML page.
 *
 * @author Jennifer Pillow <a href="mailto:jpillo2@wgu.edu">Jennifer Pillow</a>
 */
public class CustSchedController implements Initializable {
    
    GUI gui = new GUI();

    @FXML
    private ComboBox<ViewableCustomer> CustomerCBox;
    ...
    /**
     * Calls the filterAppts method when customer is chosen.
     * 
     * @param event User chooses a customer from the combo box control
     */
    @FXML
    ...
    */
    ...
    private void filterAppts(){
        String name = CustomerCBox.getSelectionModel().getSelectedItem().getCustomerName();
        String input ="SELECT * FROM appointments WHERE Customer_ID = (SELECT "
                + "Customer_ID FROM customers WHERE Customer_Name = ?)";
    ...
    public void initialize(URL url, ResourceBundle rb) {
        CustomerCBox.setItems(dao_customer.getCustomers());
